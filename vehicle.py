...
class Vehicle:
    def __init__(self):
        ...
        self.engineFrame = ttk.LabelFrame(self.ventana1, text = "Engine")
        self.engineFrame.grid(column = 0, row = 1, sticky = "WE")
        self.engineLabel = tk.Label(self.engineFrame, text = str(self.engine))
        self.engineLabel.pack()

        # añadir para widget scale luminosidad
        self.enviromentFrame = ttk.LabelFrame(self.ventana1, text = "Enviroment Light")
        self.enviromentFrame.grid(column = 0, row = 2, sticky = "WE")
        self.enviromentScale = tk.Scale(self.enviromentFrame, length= 600, from_=0, to=99, orient=tk.HORIZONTAL, command = self.enviromentUpdate)
        self.enviromentScale.pack()

        # añadir barra progreso combustible
        self.fuelFrame = ttk.LabelFrame(self.ventana1, text = "Fuel level")
        self.fuelFrame.grid(column = 0, row=3, sticky = "WE")
        self.fuelProgress = ttk.Progressbar(self.fuelFrame, orient = HORIZONTAL, length = 600, mode = "determinate") 
        self.fuelProgress.pack()
    ...
    def enviromentUpdate(self, event):
        self.environment.set_lum(self.enviromentScale.get())
    ...
    def action(self,evento):
        ...
        if evento.keysym=='r':
                ...
                self.enviromentScale.set(self.environment.get_lum())
        if evento.keysym=='f':
                self.environment.modify_lum(-10)
                self.enviromentScale.set(self.environment.get_lum())
        if evento.keysym=='w':
    ...
    def do_work(self):
        ...
        self.engineLabel.configure(text = str(self.engine))

        self.light.update()
        self.fuel.update()
        self.fuelProgress['value'] = self.fuel.get_porcentage_level()
